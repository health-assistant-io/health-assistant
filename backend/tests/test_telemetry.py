...
async def test_upload_telemetry_data(
    mock_upload, async_client: AsyncClient, mock_telemetry_data
):
    from app.main import app
    from app.core.security import get_current_user

    app.dependency_overrides[get_current_user] = override_get_current_user
    mock_upload.return_value = len(mock_telemetry_data)

    response = await async_client.post(
        "/api/v1/telemetry/data", json={"device_id": "apple_watch_1", "points": mock_telemetry_data}
    )
    assert response.status_code == 200
    assert response.json()["uploaded"] == len(mock_telemetry_data)
...
async def test_get_telemetry_data(
    mock_get_data, async_client: AsyncClient, mock_telemetry_data
):
    from app.main import app
    from app.core.security import get_current_user

    app.dependency_overrides[get_current_user] = override_get_current_user
    mock_get_data.return_value = mock_telemetry_data
