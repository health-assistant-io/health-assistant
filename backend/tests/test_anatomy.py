...
async def test_create_and_get_anatomy_structure(
    ...
    res = await async_client.post(
        "/api/v1/anatomy", json=anatomy_node_payload, headers=system_admin_headers
    )
    ...
    # 3. Get by Slug
    res_slug = await async_client.get(
        f"/api/v1/anatomy/{anatomy_node_payload['slug']}", headers=system_admin_headers
    )
...
async def test_anatomy_relations_and_traversal(
    ...
    res = await async_client.post(
        "/api/v1/anatomy/relations", json=rel_payload, headers=system_admin_headers
    )
    assert res.status_code == 200

    # 2. Fetch relationships for source (Heart should have outgoing to Cardio)
    res_related_source = await async_client.get(
        f"/api/v1/anatomy/{source_id}/related", headers=system_admin_headers
    )
    ...
    res_related_target = await async_client.get(
        f"/api/v1/anatomy/{target_id}/related", headers=system_admin_headers
    )
...
async def test_anatomy_graph_multi_hop_depth(
    ...
    await async_client.post(
        "/api/v1/anatomy/relations",
        json={"source_id": bid, "target_id": aid, "relation_type": "PART_OF"},
        headers=system_admin_headers,
    )
    await async_client.post(
        "/api/v1/anatomy/relations",
        json={"source_id": cid, "target_id": bid, "relation_type": "PART_OF"},
        headers=system_admin_headers,
    )

    # depth=1: only A and its direct neighbour B
    r1 = await async_client.get(
        "/api/v1/anatomy/graph-a/graph?depth=1", headers=system_admin_headers
    )
    ...
    r2 = await async_client.get(
        "/api/v1/anatomy/graph-a/graph?depth=2", headers=system_admin_headers
    )
    d2 = r2.json()
    ...
    r_bad = await async_client.get(
        "/api/v1/anatomy/graph-a/graph?depth=5", headers=system_admin_headers
    )
...
async def test_anatomy_import_upsert_logic(
    ...
    # 1. Initial Import
    res1 = await async_client.post(
        "/api/v1/anatomy/import", json=import_payload, headers=system_admin_headers
    )
    assert res1.status_code == 200
    stats1 = res1.json()
    ...
    res_get = await async_client.get(
        "/api/v1/anatomy/test-import-lung", headers=system_admin_headers
    )
