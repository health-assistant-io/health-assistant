...
from app.schemas.anatomy import AnatomyStructureCreate, AnatomyStructureUpdate, AnatomyRelationCreate
...
async def get_anatomy_structures(
    ...
    if category:
        query = query.where(AnatomyStructure.category == category)
...
async def create_anatomy_structure(
    ...
    db_structure = AnatomyStructure(
        **structure_in.model_dump(),
        tenant_id=tenant_id
    )
    db.add(db_structure)
    await db.commit()
...
async def get_related_structures(
    ...
    if direction in ["both", "outgoing"]:
        q = select(AnatomyRelation).options(selectinload(AnatomyRelation.target_structure)).where(AnatomyRelation.source_id == structure_id)
        if relation_type:
    ...
    if direction in ["both", "incoming"]:
        q = select(AnatomyRelation).options(selectinload(AnatomyRelation.source_structure)).where(AnatomyRelation.target_id == structure_id)
        if relation_type:
            q = q.where(AnatomyRelation.relation_type == relation_type)
        res = await db.execute(q)
...
async def get_anatomy_graph(
    ...
    edge_rows = [e for e in edge_rows if e.source_id in visible_ids and e.target_id in visible_ids]
...
def save_figure_image(slug: str, data: bytes, ext: str = "webp", kind: str = "image") -> Tuple[str, int, int]:
