...
class AnatomyImportService:
    ...
    async def import_graph(self, payload: AnatomyImportPayload) -> Dict[str, int]:
        ...
        for node_data in payload.nodes:
            try:
                result = await self.db.execute(select(AnatomyStructure).where(AnatomyStructure.slug == node_data.slug))
                existing_node = result.scalar_one_or_none()

                if existing_node:
                    existing_node.name = node_data.name
                    existing_node.category = node_data.category
        ...
        for edge_data in payload.edges:
            try:
                ...
                if not source_id or not target_id:
                    logger.warning(f"Skipping edge: Could not resolve source '{edge_data.source_slug}' or target '{edge_data.target_slug}'")
                    stats["errors"] += 1
                    continue
                ...
                if existing_edge:
                    stats["edges_updated"] += 1 # Technically a no-op right now since it has no extra attributes
                else:
                ...
                logger.error(f"Error processing anatomy edge {edge_data.source_slug} -> {edge_data.target_slug}: {e}")
                stats["errors"] += 1
