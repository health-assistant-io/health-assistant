...
- **`services/fhir_converter.py`** converts ORM `to_dict()` output (snake_case + app extensions) to canonical FHIR R4B JSON and back:
  ...
  - Cardinality fixes for R4B: `Organization.type`, `DiagnosticReport.conclusionCode`/`presentedForm`/`category` wrapped as lists; `Observation.comment` → `note`; `Observation.interpretation` string → list; `MedicationStatement.timing.repeat.timeOfDay` padded to `HH:MM:SS`.
...
- On import, each entry is validated with `fhir.resources.R4B.get_fhir_model_class(rt).model_validate(...)`; invalid entries are recorded as errors and skipped.
...
- **Upsert by id with cross-tenant remapping**: for each FHIR resource, if the imported `id` already exists **in the same tenant** → update in place; if it exists in a **different tenant** → generate a new UUID and record the remap; if it doesn't exist → insert with a new UUID. All `subject`/`patient`/`partOf` references are rewritten through the remap dict so relationships survive cross-tenant import.
...
2. The task creates its own async engine + session (`get_async_session`), runs `ExportService.run_export` / `ImportService.run_import`, and updates the job row (`PROCESSING` → `COMPLETED`/`PARTIAL`/`FAILED`).
