...
| Graphs package (`app/ai/graphs/`) | LangGraph flows (ADR-0008): one module per flow, typed State. Nodes call the gateway, never models directly. |
| Checkpoint store (`app/ai/graphs/checkpointer.py`) | Process-lifespan-held `AsyncPostgresSaver`: pool opened once in the FastAPI lifespan, boot-time idempotent `setup()`, **drain-before-close** on shutdown (in-flight graph runs finish before the pool closes). |
| Checkpoint pruning (`app/workers/ai_tasks.py::prune_checkpoint_threads`, daily beat) | The checkpoint tables have no timestamp columns, so thread age derives from the owning chat session (`thread_id` == session id): sessions idle beyond `AI_CHECKPOINT_RETENTION_DAYS` (default 30) plus orphan threads are pruned. No-op until the tables exist; HITL cards live in `chat_messages` and are never touched. |
...
Health implements the family-uniform BYOK setup contract (ai-features §15, frozen 2026-09-19; Python reference: study-assistant). One call takes a user from no provider to working defaults. **Scope-aware (health delta):** the SAME setup runs against every config layer — SYSTEM (system admins, via `/admin/system/ai-config`), TENANT (tenant admins) and USER (personal, the default). `ProviderSetupRequest.scope` + `check_scope_access` guard the layer; adoption and slot gap-fill stay strictly within the requested scope (a USER setup never touches SYSTEM/TENANT rows and vice versa), and `set-default` binds at the provider's own scope.
...
- **Explore Catalog Relations:** Multi-hop knowledge-graph traversal ("which organ does this biomarker affect?", "what diseases does this vaccine prevent?"). Token-safe: `max_depth` clamped to 1–3, optional `types` filter (e.g. only anatomy neighbors), and a `truncated` flag when the edge cap (150) is hit so the LLM can narrow further.
...
The `AIAssistanceService` runs a dynamic reasoning **graph** (defaulting to 20 iterations) that lets the LLM recursively call tools before returning a final answer. Responses are streamed via SSE to the frontend, complete with inline citations linking back to the precise clinical entities.

### Reasoning engine (ADR-0008, Phases 3.2–3.6 + 8)
A single LangGraph StateGraph (`app/ai/graphs/chat_agent.py`): `agent_step` (LLM + streamed deltas) → `tool_exec` (tool execution, HITL interception, proactive persistence) → conditional routers (`max_iterations` cap) → `finalize` (final save). Custom StateGraph rather than `create_agent`: the HITL trimmed-feedback interception and conditional save semantics are not hostable as middleware without behavior risk. The legacy `run_reasoning_loop` generator was decommissioned in Phase 8 (deleted with the `AI_AGENT_ENGINE` flag/dial); the SSE sentinel vocabulary (content deltas, `[TOOL_CALL_*]`, `[CITATION]`, `[HITL_TASK]`) is the frozen contract.

**Family flow events (Phase 6.2 dual-emit):** the graph engine additionally emits the family vocabulary — `flow_started`, `node_started`/`node_finished`, `interrupt`, `flow_finished`, `flow_failed` (with `retryable`) — alongside the legacy sentinels, via the `?flow_events` query parameter on `/stream` and `/resume` (**default `true` since Phase 8**; set `false` to suppress). The chat UI renders a live `FlowStatusCard` while a graph turn runs, hosting the interactive HITL review card in the card's detail slot; persisted cards render exactly as before once the turn completes. Legacy sentinels keep flowing until the frontend is family-events-only (separate later release).

### HITL interrupts (Phase 3.3, graph engine)
`ask_user` **pauses the turn** at the question card (terminal-card SSE): the card + assistant message are persisted proactively (survives restarts — the run is checkpointed, `thread_id` = chat session id, via the lifespan-held store), then the side-effect-free `await_user` node raises `interrupt()`. `/resume` delivers the resolution summary as `Command(resume=...)` — the answers land as the `ask_user` `ToolMessage` and the model continues the same conversation; `HitlTask` rows in `chat_messages.tasks` + idempotent 409s stay the only dedup. `propose_*` keeps continue-after-propose. Fault tolerance (ratified): `agent_step` retries transient `ConnectionError`/`TimeoutError`; `tool_exec` is pinned to a single attempt (clinical tools are not idempotent); errors bubble to the SSE error classifier. Checkpointed state is serializable-only — the LLM runnable, tools, and service ride in `config.configurable` and are re-supplied by the resume caller.
...
| `generate_anatomy_graph` | `propose_anatomy_graph_generation` | ✅ shipped (AI-defined anatomy sub-graphs via the `define_anatomy_graph` assistance task + `/anatomy/import` pipeline. Dedup at two layers: the LLM is prompted to search the existing catalog first (`search_catalogs` + `explore_catalog_relations`), and the tool embeds a pre-flight existing-graph snapshot so the generation fills gaps. The graph is generated at **proposal time** so the review card opens instantly; the frontend renders a **Graph/Table view** with proposed nodes (indigo) vs existing nodes (slate) color-coded. SYSTEM_ADMIN-gated. See [SEEDING_AND_DEMOS.md §6.4](SEEDING_AND_DEMOS.md#64-ai-driven-graph-expansion)) |
