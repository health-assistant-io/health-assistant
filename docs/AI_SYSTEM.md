...
The reasoning loop exists as two engines emitting the **identical event vocabulary** (content deltas, `[TOOL_CALL_*]`, `[CITATION]`, `[HITL_TASK]` sentinels), selected by the `AI_AGENT_ENGINE` setting (`loop` | `graph`, default `graph` since Phase 3.6; `loop` remains as rollback; tenant settings override — see the shadow dial below; unknown values fall back to `loop`).

**Family flow events (Phase 6.2 dual-emit):** the graph engine additionally emits the family vocabulary — `flow_started`, `node_started`/`node_finished`, `interrupt`, `flow_finished`, `flow_failed` — alongside the legacy sentinels, gated behind the additive `?flow_events=true` query parameter on `/stream` and `/resume` (legacy clients receive byte-identical streams). The chat UI opts in and renders a live `FlowStatusCard` while a graph turn runs, hosting the interactive HITL review card in the card's detail slot; persisted cards render exactly as before once the turn completes.
...
Both engines are wired through `chat_engine_iter`, so callers (chat stream, resume, non-streaming chat) are engine-agnostic and the SSE contract is unchanged. **Since Phase 3.6 the `graph` engine is the default**; `loop` remains available as rollback via the setting. The engine is resolved per tenant: `TenantModel.settings.ai_agent_engine` (the Phase 3.5 shadow dial — flip a single dev tenant) overrides the system DB setting, which overrides the `AI_AGENT_ENGINE` env default; invalid values fall back to `loop`.
