...
13. **Document Pull (Opt-in Document Ingestion)**: Integrations that can deliver document bytes upstream — a hospital portal integration pulling scanned lab reports, a fax-to-email gateway, a wearable companion app syncing ECG printouts — can opt-in via `supports_documents` / `pull_documents` to hand them to the platform's OCR + LLM extraction pipeline. The platform's `run_sync` pipeline routes each `DocumentPull` spec through `document_service.ingest_document_bytes` — the same canonical ingestion path the UI upload endpoint uses — and fires the OCR Celery task when requested. Two per-sync caps (`INTEGRATION_MAX_DOCS_PER_SYNC = 20`, `INTEGRATION_MAX_DOC_BYTES_PER_SYNC = 50 MiB`) protect against runaway providers. Documents can be linked to a pulled examination via `examination_external_id` (resolved against the exams pulled in the same sync) and categorized via `category_concept_slug` (resolved via the catalog). See the [SDK guide](INTEGRATIONS_SDK.md) §3.13.
