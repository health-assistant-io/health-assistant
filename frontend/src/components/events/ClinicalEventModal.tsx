...
import { 
  X, Save, Info, Plus, Trash2, Activity, Baby, AlertTriangle, Zap, Scissors, Smile, Eye, Sparkles, CheckCircle, Stethoscope, Filter, Calendar as CalendarIcon, Clock, ChevronDown
} from 'lucide-react';
import { 
  getEventTypes, 
  getEventCategories,
...
export const ClinicalEventModal: React.FC<Props> = ({ isOpen, onClose, patientId, event, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    coding_system: 'custom' as 'loinc' | 'snomed' | 'custom',
    code: ''
  });
      ...
      try {
        ...
        if (event) {
          setFormData({
            ...
            examinations: event.examinations?.map(e => ({
            ...
            coding_system: event.coding_system || 'custom',
            code: event.code || ''
          });
          setSelectedTypeId(event.type_id || '');
          
          const type = typesData.find(t => t.id === event.type_id);
          if (type && type.category_id) {
            setActiveCategoryId(type.category_id);
          }
        }
      } catch (err) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-dark-surface w-full max-w-4xl rounded-3xl shadow-2xl border border-gray-100 dark:border-dark-border overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="px-8 py-6 border-b border-gray-50 dark:border-dark-border flex items-center justify-between bg-white dark:bg-dark-surface">
          <div className="flex items-center space-x-4">
            <div className={`p-3 rounded-2xl bg-opacity-10 shadow-sm transition-all duration-500 ${selectedType ? '' : 'bg-blue-500/10'}`} style={{ backgroundColor: selectedType?.color ? selectedType.color + '20' : undefined }}>
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-dark-text tracking-tight">
                {event ? t('events.update_title') : t('events.new_title')}
              </h2>
            ...
            <div className="space-y-6">
               <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 dark:text-dark-muted">{t('events.filter_by_specialty')}</h3>
                </div>
              </div>
            ...
            <div className="space-y-8">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-5 py-4 bg-gray-50 dark:bg-dark-bg border border-transparent rounded-2xl text-gray-900 dark:text-dark-text focus:ring-4 focus:ring-blue-500/10 focus:bg-white dark:focus:bg-dark-surface outline-none font-bold transition-all"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, coding_system: e.target.value as any }))}
                    >
        ...
        <div className="px-8 py-6 bg-gray-50 dark:bg-dark-bg/50 border-t border-gray-50 dark:border-dark-border flex items-center justify-end space-x-4">
          ...
          >
            ...
            <span>{event ? t('events.update_event') : t('events.save_event')}</span>
          </button>
        </div>
      </div>
