...
export const AllergyModal: React.FC<Props> = ({ isOpen, onClose, patientId, allergy, onSuccess }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-dark-surface w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-100 dark:border-dark-border flex justify-between items-center bg-white dark:bg-dark-surface">
          ...
          <button onClick={onClose} className="p-2 hover:bg-gray-100 dark:hover:bg-dark-border rounded-full transition-colors">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          ...
          <section className="space-y-4">
            ...
            <div className="relative">
              <div className="flex gap-2">
                ...
                {isAddingNew && (
                   <button 
                    ...
                    className="px-3 bg-gray-100 dark:bg-dark-bg rounded-xl text-gray-400 hover:text-gray-600"
                   >
                     <X className="w-5 h-5" />
                   </button>
                )}
              ...
              {searchTerm.length >= 2 && !selectedCatalogItem && !isAddingNew && !allergy && (
              ...
              {isAddingNew && (
                <div className="mt-4 p-4 bg-blue-50/30 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50 dark:border-blue-900/30 animate-in zoom-in-95">
                  <h4 className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-3">{t('allergies.modal.define_category')}</h4>
              ...
              <div className="flex bg-gray-100 dark:bg-dark-bg p-1 rounded-xl">
                {(['active', 'resolved'] as const).map(s => (
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${clinicalStatus === s ? 'bg-white dark:bg-dark-surface text-blue-600 shadow-sm' : 'text-gray-50'}`}
                  >
                    {s.toUpperCase()}
                  </button>
                ))}
              ...
              <div className="flex bg-gray-100 dark:bg-dark-bg p-1 rounded-xl">
                {(['low', 'high'] as const).map(c => (
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${criticality === c ? (c === 'high' ? 'bg-red-600 text-white' : 'bg-white dark:bg-dark-surface text-blue-600 shadow-sm') : 'text-gray-50'}`}
                  >
                    {c.toUpperCase()}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {reactions.map((r, i) => (
                  <div key={i} className="flex items-center space-x-2 bg-white dark:bg-dark-surface px-3 py-1.5 rounded-full border border-gray-100 dark:border-dark-border text-sm font-medium shadow-sm">
                    ...
                    <button type="button" onClick={() => removeReaction(i)} className="text-gray-400 hover:text-red-500 ml-1">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ))}
        ...
        <div className="p-6 bg-gray-50 dark:bg-dark-border/30 border-t border-gray-100 dark:border-dark-border flex items-center justify-end space-x-3">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            ...
            disabled={submitting || !searchTerm}
            className="flex items-center space-x-2 px-8 py-2.5 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 transition-all shadow-lg shadow-blue-200 disabled:opacity-50 active:scale-95"
          >
            {submitting ? (
              ...
              <>
                ...
                <span>{allergy ? t('allergies.modal.update_record') : t('allergies.modal.save_allergy')}</span>
              </>
            )}
          </button>
        </div>
