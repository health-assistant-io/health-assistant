...
function proposalToDraft(proposed: Record<string, any> | undefined): CatalogItem {
  ...
  return {
    ...
    side_effects: Array.isArray(proposed.side_effects) ? proposed.side_effects : [],
...
export function renderCreateMedicationSummary(task: TaskInfo): React.ReactNode {
  ...
  if (p.indications) chips.push({ icon: Pill, label: String(p.indications) });
  if (Array.isArray(p.side_effects) && p.side_effects.length > 0) {
    chips.push({ icon: Pill, label: `${p.side_effects.length} side effects` });
  }
...
export const CreateMedicationDefinitionHandler: React.FC<HitlHandlerProps> = ({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const created = await addCustomMedication(createPayload as any);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 custom-scrollbar">
        <MedicationForm values={draft} onChange={handleChange} mode="create" />
      </div>
          ...
          >
            ...
            <span>
              {submitting
                ...
                : t('ai_chat.hitl.create_medication.confirm', 'Confirm & Create Definition')}
            </span>
          </button>
        </div>
      </div>
