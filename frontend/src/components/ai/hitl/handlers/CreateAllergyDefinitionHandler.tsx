...
import { AllergyForm } from '../../../catalog/forms/AllergyForm';
import { addCustomAllergen } from '../../../../services/allergyService';
...
function proposalToDraft(proposed: Record<string, any> | undefined): CatalogItem {
  ...
  return {
    ...
    typical_reactions: Array.isArray(proposed.typical_reactions)
      ? proposed.typical_reactions
      : [],
...
export function renderCreateAllergySummary(task: TaskInfo): React.ReactNode {
  const p = task.proposed_payload || {};
  const chips: { icon: React.ComponentType<{ className?: string }>; label: string }[] = [];

  if (p.name) chips.push({ icon: ShieldAlert, label: String(p.name) });
  if (p.category) chips.push({ icon: ShieldAlert, label: String(p.category) });
  if (Array.isArray(p.typical_reactions) && p.typical_reactions.length > 0) {
    chips.push({ icon: AlertTriangle, label: `${p.typical_reactions.length} reactions` });
  }
...
export function renderCreateAllergyOutcome(task: TaskInfo): React.ReactNode | null {
...
export const CreateAllergyDefinitionHandler: React.FC<HitlHandlerProps> = ({
  task,
  sessionId,
  onResolved,
  onCancel,
  ...
  const [draft, setDraft] = useState<CatalogItem>(() => proposalToDraft(task.proposed_payload));
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const created = await addCustomAllergen(createPayload as any);
      ...
      console.error('HITL create_allergy_definition confirm failed', e);
      const msg =
  ...
  const handleReject = () => {
    ...
    if (sessionId) {
      resolveHitlTask(sessionId, task.proposal_id, { status: 'dismissed' }).catch(err =>
        console.error('HITL reject allergy definition failed', err),
      );
    }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-4 custom-scrollbar">
        <AllergyForm values={draft} onChange={handleChange} mode="create" />
      </div>
      <div className="px-4 py-3 bg-gray-50 dark:bg-dark-bg/50 border-t border-gray-100 dark:border-dark-border flex items-center shrink-0">
        ...
        <div className="ml-auto flex items-center space-x-3">
          ...
          <button
            ...
            disabled={!canSubmit}
            className="px-6 py-2.5 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition-all font-bold flex items-center justify-center space-x-2 shadow-lg shadow-rose-200/50 dark:shadow-none disabled:opacity-50 active:scale-95"
          >
            {submitting ? (
            ...
            <span>
              {submitting
                ...
                : t('ai_chat.hitl.create_allergy.confirm', 'Confirm & Create Allergen')}
            </span>
          </button>
        </div>
      </div>
