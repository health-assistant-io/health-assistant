            ...
            <div className="p-6 space-y-6">
              <div className="space-y-3">
                ...
                <div className="flex bg-gray-100 dark:bg-dark-bg p-1 rounded-2xl">
                  {[
                    ...
                    <button key={type.id} onClick={() => setChartType(type.id as any)} className={`flex-1 flex items-center justify-center space-x-2 py-2 rounded-xl text-xs font-bold transition-all ${chartType === type.id ? 'bg-white dark:bg-dark-surface text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <button onClick={() => setShowReferenceRanges(!showReferenceRanges)} className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl border transition-all ${showReferenceRanges ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30 text-emerald-600' : 'bg-gray-50 dark:bg-dark-bg border-transparent text-gray-500'}`}>
...
function BiomarkerTrends() {
  ...
  const groupedMarkers = useMemo(() => 
    getGroupedData(activePerspective, activeTab, searchTerm, showAlertsOnly), 
    [getGroupedData, activePerspective, activeTab, searchTerm, showAlertsOnly, trendsData]
  );
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: activeTab === 'All' && !searchTerm && !showAlertsOnly ? totalCount : visibleTotal,
